  ...
  implements SharedPreferences.OnSharedPreferenceChangeListener
{
  ...
  /** Whether the keyboard UI is currently visible to the user. Used by
      [KeyActionService] to decide whether a physical key should act as a
      keyboard key (voice typing) or a global shortcut. */
  public static boolean isKeyboardVisible()
  {
    Keyboard2 k = sInstance;
    if (k == null)
      return false;
    if (k._overlayManager != null && k._overlayManager.isShowing())
      return true;
    try { return k.isInputViewShown(); }
    catch (Exception e) { return false; }
  }

  /** Start voice typing — the same action as the on-keyboard voice key
      ([SWITCH_VOICE_TYPING]). Called from [KeyActionService] when a physical
      key is pressed while the keyboard is showing. Returns true if handled. */
  public static boolean triggerVoiceTyping()
  {
    final Keyboard2 k = sInstance;
    if (k == null)
      return false;
    k._handler.post(new Runnable() { public void run() {
      try
      {
        if (!VoiceImeSwitcher.switch_to_voice_ime(k, k.get_imm(), Config.globalPrefs()))
          k._config.shouldOfferVoiceTyping = false;
      }
      catch (Exception e) { Logs.exn("triggerVoiceTyping", e); }
    }});
    return true;
  }
  ...
  private void refresh_config()
  {
    ...
    // container, so the candidates row stays transparent). The background can
    // be null (see Keyboard2View.applyContainerAlpha) — guard to avoid an NPE
    // that would crash the IME and lock up input device-wide.
    if (_keyboardView.getBackground() != null)
  ...
  {
    // Never let an exception escape an IME lifecycle callback: an uncaught
    // throw here takes down the system input pipeline and locks up text entry
    // device-wide, not just this keyboard.
    try
    {
      onStartInputViewImpl(info, restarting);
    }
    catch (Exception e)
    {
      Logs.exn("onStartInputView", e);
    }
  }

  private void onStartInputViewImpl(EditorInfo info, boolean restarting)
  {
  ...
  public void onCurrentInputMethodSubtypeChanged(InputMethodSubtype subtype)
  {
    ...
    {
      Logs.exn("onCurrentInputMethodSubtypeChanged", e);
    }
  }
  ...
  public void onUpdateSelection(int oldSelStart, int oldSelEnd, int newSelStart, int newSelEnd, int candidatesStart, int candidatesEnd)
  {
    ...
    try
    {
      onUpdateSelectionImpl(oldSelStart, oldSelEnd, newSelStart, newSelEnd);
    }
    catch (Exception e)
    {
      Logs.exn("onUpdateSelection", e);
    }
  }

  private void onUpdateSelectionImpl(int oldSelStart, int oldSelEnd, int newSelStart, int newSelEnd)
  {
  ...
  public void onSharedPreferenceChanged(SharedPreferences _prefs, String _key)
  {
    ...
    {
      Logs.exn("onSharedPreferenceChanged", e);
    }
  }
