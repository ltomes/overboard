...
public class Theme
{
  ...
  public static final class Computed
  {
    ...
    public static final class Key
    {
      ...
      final Paint _label_outline_paint;
      final Paint _special_label_outline_paint;
      final Paint _sublabel_outline_paint;
      final Paint _special_sublabel_outline_paint;
      final int _label_alpha_bits;
      final boolean _outline_enabled;
      final boolean _shadow_enabled;
      final float _shadow_radius_ratio;
      final int _shadow_color;
      ...
      public Key(Theme theme, Config config, float keyWidth, boolean activated, int bgColor)
      {
        ...
        _label_outline_paint = init_outline_paint(null);
        _special_label_outline_paint = init_outline_paint(_key_font);
        _sublabel_outline_paint = init_outline_paint(null);
        _special_sublabel_outline_paint = init_outline_paint(_key_font);
        _label_alpha_bits = (config.labelBrightness & 0xFF) << 24;
        _outline_enabled = config.labelOutline;
        _shadow_enabled = config.labelShadow;
        _shadow_radius_ratio = 0.15f;
        _shadow_color = 0xBB000000;
      }
      ...
      public Paint label_paint(boolean special_font, int color, float text_size)
      {
        ...
        if (_shadow_enabled)
          p.setShadowLayer(text_size * _shadow_radius_ratio, 0, 0, _shadow_color);
        else
      ...
      public Paint label_outline_paint(boolean special_font, int outlineColor, float text_size)
      {
        Paint p = special_font ? _special_label_outline_paint : _label_outline_paint;
        p.setColor(outlineColor);
        p.setTextSize(text_size);
        p.setStrokeWidth(text_size * 0.08f);
        return p;
      }

      public Paint sublabel_outline_paint(boolean special_font, int outlineColor, float text_size, Paint.Align align)
      {
        Paint p = special_font ? _special_sublabel_outline_paint : _sublabel_outline_paint;
        p.setColor(outlineColor);
        p.setTextSize(text_size);
        p.setTextAlign(align);
        p.setStrokeWidth(text_size * 0.08f);
    ...
    static Paint init_outline_paint(Typeface font)
    {
      Paint p = new Paint(Paint.ANTI_ALIAS_FLAG);
      p.setTextAlign(Paint.Align.CENTER);
      p.setStyle(Paint.Style.STROKE);
